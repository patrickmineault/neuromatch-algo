...
def agglomerate(A, group_size):
    ...
    for gs in tqdm(group_sizes):
        ...
        z = linkage(get_distance_vector(B), method="average", metric="euclidean")
...
def do_matching(M, ppl_per_group, nrounds, fake=False):
    ...
    if fake:
        A = 0.01 * np.random.randn(A.shape[0], A.shape[1])
        A[M == 1] = 1
...
def calculate_common_coauthors(users, matches, coauthors_map):
    ...
    for _, group in matches.iterrows():
        scholar_ids = users[users.user_id.isin(group["user_ids"])].inferred_id.tolist()
        ...
        counts = collections.Counter(
            list(
                itertools.chain(
                    *[list(coauthors_map.get(x).keys()) for x in scholar_ids]
                )
            )
        )
        in_common = []
...
def compute_affiliation_conflict(users):
    conflicts = []
    for i, row in users.iterrows():
        for j, row2 in users.iterrows():
            ...
            if (
                ~row.isna().Company
                and ~row2.isna().Company
                and row["Company"].strip().lower() == row2["Company"].strip().lower()
            ) or (
                ~row.isna().institution
                and ~row2.isna().institution
                and row["institution"].strip() != ""
                and row["institution"] == row2["institution"]
            ):
...
def main():
    ...
    # Do a random permutation of all the group numbers
    # Use a common seed
    np.random.seed(42)
    ...
    cois_list = compute_affiliation_conflict(users)
    for i, j in cois_list:
        M[i, j] += AFFILIATION_PENALTY

    print(f"Similar affilications matches {len(cois_list)}")
    ...
    scholar_ids = users["inferred_id"].tolist()
    for i, row in users.iterrows():
        scholar_id = row["inferred_id"]
        ...
        for c in coauthors.keys():
            if c in scholar_ids:
                # Find it and ban
                j = users[users.inferred_id == c].index[0]
                M[i, j] = 1
    ...
    for j, round in enumerate(groups):
        for i in range(int(max(round) + 1)):
            matches.append(
                {
                    "user_ids": users.iloc[np.where(round == i)].user_id.tolist(),
                    "round": j,
                    "group": i,
    ...
    # Now do NROUNDS rounds of matching.
    groups, goodness = do_matching(M, PPL_PER_GROUP, NROUNDS)

    group_nums = np.arange(max(groups[0]) + 1)
    ...
    for j, round in enumerate(groups):
        for i in range(int(max(round) + 1)):
            assert len(users.iloc[np.where(round == i)].user_id.tolist()) in (
                PPL_PER_GROUP,
                PPL_PER_GROUP - 1,
            )
            matches.append(
                {
                    "user_ids": users.iloc[np.where(round == i)].user_id.tolist(),
                    "round": round_names[j],
                    "table": group_nums[i] + 1,
                    "goodness": goodness[j][i],
                }
            )

    df_matches = pd.DataFrame(matches)
    df_matches = df_matches.sort_values(["round", "table"])
    df_matches.reset_index(drop=True, inplace=True)

    # Measure common co-authors.
    direct_coauthors, indirect_coauthors = calculate_common_coauthors(
        users, df_matches, coauthors_list
    )
    ...
    df_matches["has_indirect_coauthors"] = df_matches["indirect_coauthors"].map(
        lambda x: len(x) > 0
    )
    ...
    df_matches.to_csv("data/output/matches.csv")
