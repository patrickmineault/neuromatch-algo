...
def generate_html(matches):
    ...
    <head>
        ...
        <title>Cosyne 2023 matches</title>
    ...
    for i, row in matches.iterrows():
        ...
        btns = "\n".join(
            [
                btn_template.format(item=i, sub=x, selected=selected, status=status(x))
                for x in range(len(row["all_abstracts"]))
            ]
        )
        ...
        tabs = "\n".join(
            [
                tab_template.format(status=status(j), item=i, sub=j, content=x)
                for j, x in enumerate(row["all_abstracts"])
            ]
        )

        people = "\n".join(
            f"<li>{x} ({y})</li>"
            for x, y in zip(row["user_names"], row["user_affiliations"])
        )

        has_common_coauthors = ""
        if row["has_indirect_coauthors"] > 0:
            has_common_coauthors = (
                f"<div>Indirect coauthors: {', '.join(row['indirect_coauthors'])}</div>"
            )

        panels = f"""
        <div class='px-5 py-5 my-5mx-auto"'>
        <h1>Round {row['round']}, Table {row['table']}</h1>
        ...
        <div style="margin-left:auto; margin-right: auto; ">
            <img src="output/keywords_{row["round"]}_{row["table"]}.png"  class="mx-auto d-block" style='padding-top: 50px;padding-bottom: 50px' width='600'>
        </div>
        </div>
        """
...
def clean_keywords(top_keywords):
    ...
    kws = [
        ...
        for x in kws
        if x.lower()
        not in ("brain", "computational neuroscience", "big data", "machine learning")
    ]
    return kws


def to_na(x):
    if x.startswith("I didn't do the requested"):
        return "NA"
...
def main():
    ...
    all_abstracts = [to_na(x) for x in all_abstracts]
    ...
    for i, row in df_matches.iterrows():
        ...
        for j, n in enumerate(user_index[:-1]):
            for n2 in user_index[j + 1 :]:
                # Add to the stack
                referenced_abstracts += [
                    first_abstract.loc[(float(n), float(n2))],
        ...
        for r in referenced_abstracts:
            summarized_abstracts.append(
                ("-" + summarize_one_abstract(0, all_abstracts[r]))
                .replace("2.", "-")
                .replace("3.", "-")
            )
        ...
        k = row["round"]
        fname = f'keywords_{k}_{row["table"]}.png'
        if row["goodness"] < 0.15 and all(
            [all_abstracts[x] != "NA" for x in referenced_abstracts]
        ):
            plt.figure(figsize=(16, 8))
            print_scale = 2
            wordcloud = WordCloud(
                font_path="/usr/local/share/fonts/FiraSansCondensed-Regular.ttf",
                colormap="Greys_r",
                max_font_size=160,
                ...
                height=600 * print_scale,
            ).generate_from_frequencies(words)

            wordcloud.to_file(fname)
            os.rename(fname, f"html/output/{fname}")
        else:
            # Create an empty white PNG instead of confusing people with bad
            # keywords.
            plt.figure(figsize=(16, 8))
            plt.axis("off")
            plt.savefig(f"html/output/{fname}")
        plt.close(plt.gcf())

    total_num = len(top_keywordss)
    # And now generate the HTML for this.
    df_matches.loc[: total_num - 1, "top_keywords"] = [
        ", ".join(x) for x in top_keywordss
    ]
    df_matches.loc[: total_num - 1, "all_abstracts"] = all_abstractss

    name_map = {x["user_id"]: x["fullname"] for i, x in users.iterrows()}
    df_matches["user_names"] = df_matches.user_ids.map(
        lambda x: [name_map[y] for y in x]
    )
    name_map = {x["user_id"]: x["Company"] for i, x in users.iterrows()}
    ...
    for j, row in df_matches.iterrows():
        for i, (user_id, name) in enumerate(zip(row["user_ids"], row["user_names"])):
            df_matches.loc[j, f"user_id_{i}"] = user_id
            df_matches.loc[j, f"user_name_{i}"] = name
    df_matches["wordcloud"] = (
        "keywords_"
        + df_matches["round"].astype(str)
        + "_"
        + df_matches.table.astype(str)
        + ".png"
    )

    df_matches.to_json("data/output/matches_with_annotations.json")
    df_matches.indirect_coauthors = df_matches.indirect_coauthors.map(
        lambda x: ", ".join(x)
    )
    df_matches = df_matches.drop(
        columns=["user_ids", "user_names", "has_indirect_coauthors"]
    )
