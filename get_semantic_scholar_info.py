...
def find_best_author_match(search_results, user):
    ...
    for r in search_results['data']:
        the_papers = [clean_none(x.get('title')) + ' ' + clean_none(x.get('abstract')) for x in r['papers'][:10]]
        if len(the_papers) == 0:
            scores.append(-1)
            continue
...
def main():
    ...
    for _, user in users.iterrows():
        fullname = user['fullname'].replace('-', ' ').replace('.', ' ').replace("'", " ").replace(' ', '+')

        q = cached_request(f'https://api.semanticscholar.org/graph/v1/author/search?query={fullname}&fields=affiliations,papers.fieldsOfStudy,papers.title,papers.abstract,papers.authors&limit=50')
        ...
        user_matches[user['user_id']] = {'best_score' : best_scores[-1],
                                        'inferred_id': inferred_ids[-1]}
        i += 1

    users['manual_scholar_id'] = users.mindMatchScholarID.str.split('/').map(lambda x: x[-1] if not isinstance(x, float) and x[-1][0] in '0123456789' else np.nan)
    ...
    # third coauthor.
    users['consensus_scholar_id'] = np.where(users.manual_scholar_id.isna(),
                                                np.where(users.inferred_score > .4, 
                                                        users.inferred_scholar_id,
                                                        np.nan),
    ...
    for i, user in tqdm(users.iterrows()):
        ...
        q = cached_request(f'https://api.semanticscholar.org/graph/v1/author/{user.consensus_scholar_id}?fields=papers.authors')
        
        min_paper_len = max([min([len(x['authors']) for x in q['papers']]), 25])
        coauthors = itertools.chain(*[x['authors'] for x in q['papers'] if len(x['authors']) <= min_paper_len])
