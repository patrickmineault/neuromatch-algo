...
def paged_cached_request(url):
    ...
    while "next" in results:
        ...
        if results is None:
            results["next"] = results["next"] + page_size
            continue
...
def infer_author_id_from_google(url):
    ...
    for pub in data["publications"]:
        ...
        distances = [
            (levenshtein.distance(x, author_name), i) for i, x in enumerate(author_list)
        ]
...
def infer_author_id(data):
    ...
    if "google_scholar" in data and data["google_scholar"] != "":
        print("Inferring from Google Scholar")
        author_id = infer_author_id_from_google(data["google_scholar"])
        if author_id is not None:
            print(f"Inferred From Google Scholar {author_id}")
            return author_id
